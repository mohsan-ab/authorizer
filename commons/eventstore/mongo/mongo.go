...
func NewClient(host string, port int, db string) (eventsource.EventStore, error) {
...
func (c *Client) save(events []eventsource.Event, version int, safe bool) error {
	if len(events) == 0 {
		return nil
	}
...
func (c *Client) SafeSave(events []eventsource.Event, version int) error {
	return c.save(events, version, true)
}

//Save the events ensuring the current version
func (c *Client) Save(events []eventsource.Event, version int) error {
	return c.save(events, version, false)
}

//Load the stored events for an AggregateID
func (c *Client) Load(aggregateID string) ([]eventsource.Event, error) {
	...
	events = make([]eventsource.Event, len(aggregate.Events))
	register := eventsource.NewEventRegister()
	...
	for i, dbEvent := range aggregate.Events {
		...
		// Translate dbEvent to eventsource.Event
		events[i] = eventsource.Event{
